...
/// Learn both character fragments (as is done in the special low exposure mode) as well as
...
/// Score multiplier for word matches which have good case and are frequent in the given language
/// (lower is better).
///@brief Type double Default 1.0
...
/// Which OCR engine(s) to run (Tesseract, LSTM, both). Defaults to loading and running the most
/// accurate available.
...
/// How many times worse certainty does a superscript position glyph need to be for us to try
/// classifying it as a char with a different baseline?
...
/// Maximum number of prunable (those for which PrunablePath() is true) entries in each viterbi list
...
/// Remove and conditionally reassign small outlines when they confuse layout analysis, determining
/// diacritics vs noise
///@brief Type BOOL Default true
...
/// Maximum number of different character choices to consider during permutation. This limit is
/// especially useful when user patterns are specified, since overly generic patterns can result in
...
/// A superscript scaled down more than this is unbelievably small.  For example, 0.3 means we
/// expect the font size to be no smaller than 30% of the text line font size.
///@brief Type double Default 0.4
...
/// Don't use any alphabetic-specific tricks. Set to true in the traineddata config file for scripts
...
/// Score multiplier for glyph fragment segmentations which do not match a dictionary word (lower is
/// better).
///@brief Type double Default 1.25
...
/// Minimum bottom of a character measured as a multiple of x-height above the baseline for us to
/// reconsider whether it's a superscript.
///@brief Type double Default 0.3
...
/// Only initialize with the config file. Useful if the instance is not going to be used for OCR but
/// say only for layout analysis.
///@brief Type BOOL Default false
...
/// Maximum top of a character measured as a multiple of x-height above the baseline for us to
/// reconsider whether it's a subscript.
///@brief Type double Default 0.5
...
/// To avoid overly small denominators use this as the floor of the probability returned by the
/// ngram model
...
/// Score multiplier for poorly cased strings that are not in the dictionary and generally look like
/// garbage (lower is better).
...
/// Use two different windows for debugging the matching: One for the protos and one for the
/// features.
///@brief Type BOOL Default FALSE
...
/// Make AcceptableChoice() always return false. Useful when there is a need to explore all
/// segmentations
///@brief Type BOOL Default false
...
/// Page seg mode: 0=osd only, 1=auto+osd, 2=auto, 3=col, 4=block, 5=line, 6=word, 7=char (Values
/// from PageSegMode enum in publictypes.h)
...
/// Exposure value follows this pattern in the image filename. The name of the image files are
/// expected to be in the form [lang].[fontname].exp[num].tif
...
/// Each bounding box is assumed to contain ngrams. Only learn the ngrams whose outlines overlap
/// horizontally.
///@brief Type BOOL Default false
...
/// Score penalty (0.1 = 10%) added if there are subscripts or superscripts in a word, but it is
/// otherwise OK.
///@brief Type double Default 0.125
...
/// For example, a value of 0.6 means we want to reduce badness of certainty by 40%
...
/// force associator to run regardless of what enable_assoc is. This is used for CJK where component
